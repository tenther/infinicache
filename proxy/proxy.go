...
var (
    ...
    clientaddr = flag.String("client-addr", "", "Address for client to connect to")
    lambdaaddr = flag.String("lambda-addr", "", "Address for lambda to connect to")
...
func main() {
    ...
    log.Info("======================================")
    clientLis, err := net.Listen("tcp", fmt.Sprintf("%s:%d", *clientaddr, global.BasePort))
    if err != nil {
        log.Error("Failed to listen clients: %v", err)
        os.Exit(1)
        return
    }
    lambdaLis, err = net.Listen("tcp", fmt.Sprintf("%s:%d", *lambdaaddr, global.BasePort+1))
    if err != nil {
