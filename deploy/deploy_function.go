...
const (
    ROLE = "arn:aws:iam::683858342714:role/infinicache-lambda-role"
)
...
var (
    ...
    subnet = []*string{
        aws.String("subnet-05da0748"),
        aws.String("subnet-06a36408"),
    }
    securityGroup = []*string{
        aws.String("sg-09ceba5a800219dae"),
    }
)
    ...
    if err != nil {
        if aerr, ok := err.(awserr.Error); ok {
            ...
            case lambda.ErrCodeCodeStorageExceededException:
                fmt.Println(lambda.ErrCodeCodeStorageExceededException, aerr.Error())
            default:
                fmt.Println("Unknown error:", aerr.Error())
            }
        } else {
            // Print the error, cast err to awserr.Error to get the Code and
            // Message from an error.
